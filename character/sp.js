	...
	return {
		...
		characterSort:{
			sp:{
				...
				sp_default:["sp_diaochan","sp_zhaoyun","sp_sunshangxiang","sp_caoren","sp_jiangwei","sp_machao","sp_caiwenji","jsp_guanyu","jsp_huangyueying",
							"sp_pangde","sp_jiaxu","yuanshu",'sp_zhangliao','sp_ol_zhanghe','sp_menghuo','ol_zhouqun'],
				sp_qifu:['guansuo','baosanniang',"caoying",'panshu',"caochun","yuantanyuanshang",'caoshuang','wolongfengchu','fengfangnv','caoxiancaohua',
						'jin_zhouchu'],
		...
		characterIntro:{
			ol_caoxi:'曹真之子，曹爽之弟。曹爽掌权后，受封中领军，掌握禁兵，封安乡侯。曹爽及诸兄弟轻视司马懿，恣意妄为，经常外出狩猎，曹羲屡次劝谏，不被采纳。249年，司马懿发动高平陵政变，被夷三族。',
		...
		skill:{
			olfeiyi1:{
				...
				filter:function(event,player){
					...
					return !player.hasSkill('olfeiyi1_'+num);
				},
				...
				content:function(){
					'step 0'
					player.addTempSkill('olfeiyi1_'+(cards.length?'0':'1'),'phaseUseAfter');
					if(cards.length){
						player.draw(3)
						event.finish();
					}
					...
					player.chooseToDiscard('h','技能：弃置至少一半手牌',[Math.floor(player.countCards('h')/2),Infinity],true).set('ai',card=>{
						var player=_status.event.player;
						if(player.hasSkill('olfeiyi2')&&player.countCards('h')-ui.selected.cards.length>1) return 1/(get.value(card)||0.5);
						if(!player.hasSkill('olfeiyi2')&&ui.selected.cards.length<Math.floor(player.countCards('h')/2)) return 1/(get.value(card)||0.5);
						return 0;
					});
				},
				subSkill:{
			...
			olfeiyi2:{
				...
				filter:function(event,player){
					...
					if(player.hasSkill('olfeiyi2_0')&&player.hasSkill('olfeiyi2_1')) return false;
					var evt=event.getl(player);
					return evt&&evt.player==player&&evt.hs&&evt.hs.length>0;
				},
				prompt2:function(event,player){
					var str='展示最后一张手牌并摸一张牌';
					if(!player.hasSkill('olfeiyi2_0')||!player.hasSkill('olfeiyi2_0')){
						str+='，然后令本回合使用点数';
						if(!player.hasSkill('olfeiyi2_0')) str+='大于';
						if(!player.hasSkill('olfeiyi2_0')&&!player.hasSkill('olfeiyi2_0')) str+='或';
						if(!player.hasSkill('olfeiyi2_1')) str+='小于';
				...
				content:function(){
					...
					player.draw();
					'step 2'
					if(player.hasSkill('olfeiyi2_0')) event._result={index:1};
					else if(player.hasSkill('olfeiyi2_1')) event._result={index:0};
					else{
					...
					'step 3'
					var skill='olfeiyi2_'+result.index;
				...
				subSkill:{
					'0':{
						...
						filter:function(event,player){
							...
							return typeof num=='number'&&player.getStorage('olfeiyi2_0').some(numx=>num>numx);
						},
						forced:true,
						content:function(){
							trigger.effectCount++;
					...
					'1':{
						...
						filter:function(event,player){
							var num=get.number(event.card,player);
							return typeof num=='number'&&player.getStorage('olfeiyi2_1').some(numx=>num<numx);
						},
						forced:true,
						content:function(){
							trigger.effectCount++;
			...
			ollvboshe1:{
				...
				prompt2:function(event,player){
					return '令'+get.translation(lib.skill.ollvboshe1.logTarget(event,player))+'进行一个额外回合';
				},
				logTarget:function(event,player){
					var storage=player.storage.ollvboshe1;
					return storage?event.player:game.zhu;
				},
				content:function(){
					var target=lib.skill.ollvboshe1.logTarget(trigger,player);
					player.changeZhuanhuanji('ollvboshe1');
					target.insertPhase();
				},
			},
			oltianhou:{
				...
				content:function(){
					...
					if(result.bool){
						...
						game.log(target,'夜观天象，今天是','#g【'+get.translation(event.weather_skill)+'】');
					}
				},
			...
			olchenshuo:{
				...
				content:function(){
					...
					if(result.bool){
						...
						player.showCards(result.cards,get.translation(player)+'发动了#g'+get.translation('olchenshuo'));
			...
			olrunwei:{
				...
				content:function(){
					...
					player.chooseControl('弃牌','摸牌','cancel2').set('choiceList',[
			...
			olkuansai:{
				...
				filter:function(event,player){
					return event.isFirstTarget&&event.targets.length>player.hp;
				},
				direct:true,
				content:function(){
					'step 0'
		...
		translate:{
			...
			olliewei:'裂围',
			olliewei_info:'当你杀死对手时，你可以摸三张牌。',//新版军八抽象描述，和不明白希腊字母的星猴一样
			ol_zhouqun:'周群',
			oltianhou:'天候',
			...
			ol_feiyi:'费祎',
			olfeiyi1:'技能',
			olfeiyi1_info:'出牌阶段各限一次，若你的手牌数为：①奇数，你可以摸三张牌，然后弃置至少一半手牌（向下取整）；②偶数，你可以弃置至少一半手牌，然后摸三张牌。',
			olfeiyi2:'技能',
			olfeiyi2_info:'每回合每项限一次，当你的手牌数变为1后，你可以展示该牌并摸一张牌，然后你选择一项：①本回合使用点数大于该牌的点数的牌额外结算一次；②本回合使用点数小于该牌的点数的牌额外结算一次。',
			ol_lvboshe:'吕伯奢',
			ollvboshe1:'技能',
			ollvboshe1_info:'农民的回合结束时：阴，你可以令地主进行一个额外回合；阳，你可以令其进行一个额外回合。',
